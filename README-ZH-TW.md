# PowerShell 批處理智慧壓制工具

為自動化「編碼參數配置」而開發的腳本軟體，解決了影片壓縮軟體需要手動配置編碼參數、以及純命令行操作需要手動輸入路徑的痛點。本工具會根據電腦硬體規格、影片屬性、程序格式、腳本補充、用戶需求等規則為前提配置出編碼命令（批處理）。此工具幾乎消滅了影片壓製的學習成本與配置操作的時間成本，快點上手試試吧~
...
- [x] 按步驟順序拆分的腳本：復用一切已完成的工作，最大限度避免從頭開始重做
- [x] 深度訂製編碼參數：根據電腦硬體規格，用戶需求，影片屬性實現盡可能符合需求的壓制配置
...
3. 解壓下載好的壓縮包
4. 運行步驟 1 從而完成基本環境檢測
    - Windows 10: ![Run-In-Windows-10-zh](bbenc-run-win10-zh.png)
...
- 步驟 4:
  ...
  - 簡化了代碼
- 步驟 2：
  - 修正了批次部分運行 `cmd /k` 過早，導致 `exit` 命令被輸入到錯誤 cmd 進程的問題

**v1.7.2**
- 文件刪除操作交互邏輯變更——輸入 y 確認，輸入 q 取消，Enter 循環的操作邏輯
- 步驟 4 簡化了程式碼，無功能性變更
...
- 解決了檔案名本身觸發 ffprobe 分析 JSON 解析崩潰的錯誤（臨時切換到 UTF-8 文本編碼）

**v1.6.5**
- 步驟 4：
  - 美化終端輸出資訊排版
  - 限制 ffmpeg log 資訊，避免 ffmpeg 進度條與編碼器進度條衝突
...
**v1.6.0**
- 步驟 2 改良了導入系統
  - 添加了腳本所在目錄下的 ffmpeg、avs2yuv、avs2pipemod、x264、x265、SvtAv1EncApp 的模糊匹配功能
  - 添加了 PATH 路徑的 ffmpeg、avs2yuv、avs2pipemod、x264、x265、SvtAv1EncApp 的模糊匹配功能
