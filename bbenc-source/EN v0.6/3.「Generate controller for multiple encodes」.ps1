...
Switch ($is_mov) {
    $true {
        ...
        Invoke-Expression $parsProbe > "$env:USERPROFILE\temp_v_info_is_mov.csv" #File is saved to C drive because most Windows PC has only has 1 disk
    ...
    $false{
        ...
        Invoke-Expression $parsProbe > "$env:USERPROFILE\temp_v_info.csv"        #File is saved to C drive because most Windows PC has only has 1 disk
