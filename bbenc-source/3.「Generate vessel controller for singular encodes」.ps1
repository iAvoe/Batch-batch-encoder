...
Read-Host "Multiple encoding mode only specifies the path to import files to encode, therefore manually adding files into generated controller batch is needed`r`nx264 usually comes with lavf, unlike x265, therefore x265 usually exports .hevc raw-streams instead of .mp4. Press Enter to proceed"
...
Do {Switch (Read-Host "`r`nChoose how to specify filename of encoding exports [A: Input manually | B: Copy from an existing file | C: $vidEXP]`r`nNote that PowerShell misinterprets square brackets next to eachother, e.g., [some][text]. Space inbetween them is required") {
        a { $vidEXP=Read-Host "`r`nSpecify filename to export (w/out file extension). Under multi-encoding mode, place episode conter `$serial in desired location.`r`n`r`n`$serial should be padded from trailing alphabets. e.g., [Zzz] Memories – `$serial (BDRip 1764x972 HEVC)"
...
#} While (($vidEXP.Contains("`$serial") -eq $false) -or (namecheck($vidEXP) -eq $false)) #Multi-encoding only, comment out under single-encode mode
} While (($vidEXP.Contains("`$serial") -eq $true) -or (namecheck($vidEXP) -eq $false)) #Single-encoding only, comment out under multi-encode mode
