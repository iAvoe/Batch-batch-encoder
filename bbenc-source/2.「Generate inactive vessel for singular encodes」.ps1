...
#        if ($qty -eq 0) {"Non-integer or no value was entered"} elseif ($qty -gt 15625) {"Overload : Greater than 15625 individual encodes"}
...
if ($ENCops -eq "a") {
    ...
    Do {Switch (Read-Host "Choose [A: Copy from a file | B: Input] as the temporary MP4's filename") {
            a { Write-Output "Opening a selection window to [get filename from a file]"
                ...
                Write-Output "`r`nIn multi-encode mode, choosing A will added a trailing counter in filename`r`n"
            }
            b { Do {$vidEXP=Read-Host "`r`nSpecify the filename w/out extension. Do not specify episode conter in single-encode mode.`r`nSpace is needed inbetween 2 square brackets, e.g., [YYDM-11FANS] [Yuru Yuri 2] 03 [BDRIP 720P]"
                #    if (($vidEXP.Contains("`$serial") -eq $false) -or (namecheck($vidEXP) -eq $false)) {Write-Warning "Missing variable `$serial under multi-encode mode; No value entered, Or intercepted illegal characters / | \ < > : ? * `""}
                #} While (($vidEXP.Contains("`$serial") -eq $false) -or (namecheck($vidEXP) -eq $false)) #Multi-encoding only, cumment out under single-encode mode
                    if (($vidEXP.Contains("`$serial") -eq $true) -or (namecheck($vidEXP) -eq $false)) {Write-Warning "Detecting variable `$serial in single-encode mode; No value entered, Or intercepted illegal characters / | \ < > : ? * `""}
                } While (($vidEXP.Contains("`$serial") -eq $true) -or (namecheck($vidEXP) -eq $false)) #Single-encoding only, cumment out under multi-encode mode
    ...
    Write-Output "√ Prepared exporting filename $vidEXP`r`n"
...
::「Debug」Comment out with REM (+compatiblity with precentage signs) during 
REM @echo %ffmpegParA%
REM @echo %ffmpegVarA%
REM @echo %x265ParA%
REM @echo %x265VarA%
...
::「Encode」Comment out during debugging
"+$ENCwrt+"

::「Temp-MP4-mux」Works with x265 (downstream of pipe)
"+$MUXwrt+"

::Choose 「y:Continue/n:Pause/z:END」Continuing after 5s sleep, false input are blocked by choice statement, pause allows continue. This comment needs to be lifted 1 line above commands below to prevent weird issues
...
::「Encode」Comment out during debugging. avs2pipemod的pipe左侧没有`"-`"
