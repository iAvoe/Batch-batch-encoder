...
if ($ENCops -eq "a") {
    ...
    Do {Switch (Read-Host "选择导出临时封装的文件名[A: 从现有文件复制 | B: 手动填写]") {
            a { Write-Output "已打开[复制文件名]的选择窗"
                ...
                Write-Output "`r`n大批量模式下, 选项B, C会在末尾添加序号`r`n"
            }
            b { Do {$vidEXP=Read-Host "`r`n填写文件名(无后缀), 大批量模式下要求于集数变化处填 `$serial, 并隔开`$serial后的英文字母, 两个方括号间要隔开. 如[YYDM-11FANS] [Yuru Yuri 2]`$serial[BDRIP 720P]"
