...
# 需要結合影片數據統計的參數，管道參數已經在先前腳本完成，這裡不寫
...
function Get-EncodeOutputName {
    ...
    while ($true) {
        ...
        elseif ($inputOp -eq 'b') { # 3-3：選項 b
            Show-Info "手動輸入..."
            Write-Host " 兩個方括號間必須要有字元隔開，不要輸入特殊符號"
...
function Get-EncodingIOArgument {
    ...
    if (!$isImport -and !$isx264 -and $script:interlacedArgs.isInterlaced) {
        if ($isSVTAV1) { # Continue on error
            Show-Warning "Get-EncodingIOArgument: SVT-AV1 不支持隔行掃描源"
        }
        if ($isx265) {
    ...
    if ($isImport) { # 導入模式
        switch -Wildcard ($program) {
            ...
            # $sourceJson.sourcePath 只有 .vpy 或 .avs 單個源，而先前步驟允許選擇多種上遊程序
            ...
            'vspipe' {
                if ($sourceExtension -ne '.vpy') {
                    ...
                    Show-Debug "vspipe 線路缺乏 .vpy 腳本源，嘗試匹配新路徑: $(Split-Path $newSource -Leaf)"
                    if (Test-Path -LiteralPath $newSource) {
                        $source = $newSource
                        $quotedInput = Get-QuotedPath $source
                        Show-Success "已成功切換源到 $newSource"
                    }
                    else { Show-Debug "vspipe 線路的導入需手動糾正" }
                }
                # 返回輸入路徑和隔行掃描參數
            ...
            { $_ -in @('avs2yuv', 'avs2pipemod') } {
                if ($sourceExtension -ne '.avs') {
                    ...
                    Show-Debug ($_ + " 線路缺乏 .avs 腳本源，嘗試匹配新路徑: $(Split-Path $newSource -Leaf)")
                    if (Test-Path -LiteralPath $newSource) {
                        $source = $newSource
                        $quotedInput = Get-QuotedPath $source
                        Show-Success "已成功切換源到 $newSource"
                    }
                    else { Show-Debug ( $_ + " 線路的導入需手動糾正") }
                }
                # 返回輸入路徑和隔行掃描參數
                if ($iArg) { return "$quotedInput $iArg" }
                else { return $quotedInput }
        ...
        switch ($program) {
            ...
            default { throw "未識別的導出程序：$program" }
        }
    }
...
function Get-Keyint { 
    ...
    if (($isx264 -and $isx265) -or ($isx264 -and $isSVTAV1) -or ($isx265 -and $isSVTAV1)) {
        throw "Get-Keyint：參數異常，一次只能給一個編碼器配置參數"
    }
    ...
    if ($askUser) {
        ...
        do { # 默認多軌剪輯的的解碼占用為關鍵幀間隔取和，但實際情況下，解碼占用取決於硬體解碼器的數量，所以僅設為兩倍
            ...
            $userSecond =
                Read-Host " 大致範圍：[低功耗/多軌剪輯：6-7 | 一般（不確定則用）：8-10 | 高：11-13+ ]"
...
function Get-FrameCount {
    ...
    if ($null -eq $vidStream) {
        throw "無法解析 ffprobe JSON 或找不到影片串流資訊，建議重新運行步驟 3 腳本"
    }
    # 嘗試 nb_frames 欄位、tags 中的 NUMBER_OF_FRAMES
    $frameCount =
    ...
    if ($showWarning) { # 從外部控制只顯示一次
        Show-Warning 'Get-FrameCount：影片總幀數數據不存在或被刪除，將無法估計編碼進度與 ETA'
    }
    return ""
}
...
function Get-InputResolution {
    ...
    if ($null -eq $w -or $null -eq $h) {
        throw "Get-InputResolution：影片元數據缺少幀大小（寬高）資訊"
    }
...
function Get-FPSParam {
    ...
    if ($switchedOn.Count -eq 0) {
        throw "Get-FPSParam：必須指定一個程序，如啟用 -isffmpeg"
    }
    if ($switchedOn.Count -gt 1) {
        throw "Get-FPSParam：最多指定一個程序，當前指定了: $($switchedOn -join ', ')"
    }

    if ([string]::IsNullOrWhiteSpace($fpsString)) {
        throw "Get-FPSParam：影片元數據缺少幀率資訊"
    }
...
function Get-ColorSpaceSEI {
    ...
    $result = @()
    if (($isx264 -and $isx265) -or ($isx264 -and $isSVTAV1) -or ($isx265 -and $isSVTAV1)) {
        throw "Get-ColorSpaceSEI：參數異常，一次只能給一個編碼器配置參數"
    }
    ...
    elseif ($isSVTAV1) {
        ...
        $c = switch ($ColorMatrix) {
            ...
            ictcp        { 14 }
            default { 
                Show-Warning "Get-ColorSpaceSEI：未知矩陣格式：$ColorMatrix，使用默認（bt709）"
                1
            }
        }
        ...
        $t = switch ($Transfer) {
            ...
            hlg             { 18 }
            default { 
                Show-Warning "Get-ColorSpaceSEI：未知傳輸特質：$Transfer，使用默認（bt709）"
                1
            }
        }
        ...
        $p = switch ($Primaries) {
            ...
            ebu3213    { 22 }
            default {
                Show-Warning "Get-ColorSpaceSEI：未知原色色系：$Primaries，使用默認（bt709）"
                1
            }
        }
        ...
        Show-Warning "Get-ColorSpaceSEI：未指定編碼器，跳過色彩矩陣，傳輸特質與原色色系參數配置"
        return ""
    }
...
function Main {
    ...
    Show-Info "正在讀取步驟 3 的影片分析結果..."
    # 1. 自動尋找最新的 ffprobe json，並讀取影片資訊
    $ffprobeJsonPath = 
    ...
    # Show-Debug "矩陣格式：$($videoStream.color_space)；傳輸特質：$($videoStream.color_transfer)；原色色系：$($videoStream.color_primaries)"
    ...
    while (-not $templateBatch) {
        $templateBatch = Select-File -Title "選擇 encode_template.bat 批處理" -BatOnly
...
"@
    ...
    elseif ($batchContent -match $zhtwAnchor) {
        ...
        throw "未在步驟 2 生成的模板中找到參數占位符，請重新運行步驟 2 腳本"
    }
    ...
    try {
        ...
        Show-Info "批處理文件使用說明："
        Write-Host "1. 直接運行該 encode_task_final.bat 以開始編碼。"
        Write-Host "2. 只要編碼工具不變，就可以保留 encode_template.bat，"
        Write-Host "   以便下次編碼跳過步驟 2，直接在本步驟導入 encode_template.bat"
