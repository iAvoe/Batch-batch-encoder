...
Function whichlocation($startPath='DESKTOP') {
    ...
    if (($endPath.SubString($endPath.Length-1) -eq "\") -eq $false) {$endPath+="\"}#只有路径才加"\"
    return $endPath
}
...
#「ffprobeC1」ffprobe获取视频总帧数并赋值到$x264/5VarA中, 唯单文件版可用
...
Do {Switch (Read-Host "`r`n选择导出压制结果的文件名[A: 手动填写 | B: 选择文件并拷贝 | C: $vidEXP]`r`n注意PowerShell默认紧挨的方括号为一般表达式, 如[随便][什么]之间要隔开") {
        a { $vidEXP=Read-Host "`r`n填写导出用的文件名(无后缀), 仅大批量模式下于序号处填 `$serial. `$serial后不能紧挨英文字母. 如 [Zzz] Memories – `$serial (BDRip 1764x972 HEVC)"
