...
if ($mode -eq "m") {
    ...
    [int]$qty=0 #从0而非1开始数
    Do {[int]$qty = (Read-Host -Prompt "指定[生成压制批处理]的整数数量 (1~15625)")
        if ($qty -eq 0) {nonintinputerr} elseif ($qty -gt 15625) {Write-Error "× 编码次数超过15625"; pause; exit}
    } While ($qty -eq 0)
...
Read-Host "`r`n将打开[导出待调用批处理]的路径选择窗. [Enter]继续"
...
Do {Do {
    ...
    if ((Read-Host "`r`n√ [Enter]导入更多线路(推荐), 输入[y][Enter]以进行下一步") -eq "y") {$impEND="y"} else {$impEND="n"} #用户选择是否完成导入操作并退出
} While ($impEND -eq "n")
#生成一张表来表示所有已知路线
...
For ($s=0; $s -lt $qty; $s++) {
    ...
    #循环中的keyRoute和altRoute在下方激活$sChar变量
    #由于单纯激活$sChar变量后, $altRoute Array只会生成换行失败的多行文本, 所以先pipe到Out-String变成多行字符串, 才能激活$sChar变量
...
Write-Output "`r`n完成，只要线路不变，步骤3生成的各种批处理（步骤4）就可以一直调用enc_0S.bat / enc_X.bat"
pause