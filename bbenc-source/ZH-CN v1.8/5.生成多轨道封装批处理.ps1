...
.DESCRIPTION
    封装对过视音频、字幕字体批处理的工具，运行批处理即可完成封装操作。本地化由繁化姬實現：https://zhconvert.org 
.AUTHOR
        ...
        if ($vData -and $IsFirstVideo) {
            ...
            if ($isCurrentFrameRateValid) {
                ...
                switch ($choice.ToLower()) {
                    ...
                    '3' { # 选择常见帧率
                        Show-Warning "帧率必须与源完全一致，否则视频无法正确播放"
...
function Main {
    ...
    Show-Warning " ffmpeg 正在弃用 MP4 时间码（pts）生成功能，届时 MP4 格式选项将不可用"
