...
Write-Output "√ 选择了 $encEXT"

#「启动K」选择导出压制结果文件名的多种方式, 集数变量$serial于下方的循环中实现序号叠加
[int]$serial=1
$vidEXP=[io.path]::GetFileNameWithoutExtension($impEXTc)
Do {Switch (Read-Host "`r`n选择导出压制结果的文件名[A: 手动填写 | B: 选择文件并拷贝 | C: $vidEXP]`r`n注意PowerShell默认紧挨的方括号为一般表达式, 如[随便][什么]之间要隔开") {
        ...
        c { $vidEXP+='_$serial'} #使用单引号防止$serial变量被激活. 单文件模式下移除
        default {Write--Warning "输入错误, 重试`r`n"}
    }
} While (($vidEXP.Contains("$serial")) -eq $false)
Write-Output "√ 写入了导出文件名 $vidEXP`r`n"

#「启动L, M」1: 根据选择x264/5来决定输出.hevc/.mp4. 2: x265下据cpu核心数量, 节点数量添加pme/pools. x265一般不自带libav(不支持导出MP4), x264一般带libav但不支持pme和pools
...
elseif ($ENCops -eq "a") {
    ...
    if ($cores -gt 21) {$pme="--pme"; Write-Output "检测到处理器核心数达22`r`n√ 已添加x265参数: --pme`r`n"}
    ...
    if ($procNodes -gt 1) {Write-Output "检测到安装了 $procNodes 颗处理器`r`n√ 已添加x265参数: $pools`r`n"}
} 
...
For ($s=0; $s -lt $qty; $s++) {
    ...
    [string]$serial=($s).ToString($zroStr) #补零处理, 所以不能直接用$s作为集数变量. $serial从int转string以实现序号补0操作
    $vidEXX+=$ExecutionContext.InvokeCommand.ExpandString($vidEXP) #将$serial作为变量而不是默认的字符启用需要expand. 前面使用了单引号来避免$serial被作为变量启用

    $tempMuxOut=$vidEXX[$s]+".mp4" #大批量封装模式下的临时封装赋值方案. 含$serial故不使用$sChar
    ...
    $x+=1 #循环会自动实现$s+=1, 但debug时要手动调用
}
#获得循环构建好的序列, 赋值到字符串中实现换行写入. 每行命令都对应一趟独立编码任务批处理运行所需的信息
[string]$ffmpegVarWrap=$ffmpegVarSChar;$ffmpegVarWrap=$ffmpegVarWrap -replace " @set", "@set" #由于序列转字符串会生成" @set xxx", 所以通过替换改回"@set xxx"
...
::「debug与测试」x265变化参数, 平时注释掉, 末尾不加空格
...
if ($IMPchk -eq "a") {$exptPath+="4A.M.「编码主控」.bat"
} elseif ($IMPchk -eq "b") {$exptPath+="4B.M.「编码主控」.bat"
} elseif ($IMPchk -eq "c") {$exptPath+="4C.M.「编码主控」.bat"
} elseif ($IMPchk -eq "d") {$exptPath+="4D.M.「编码主控」.bat"}

Write-Output "`r`n正在生成 $exptPath"
[IO.File]::WriteAllLines($exptPath, $ctrl_gen, $utf8NoBOM) #强制导出utf-8NoBOM编码
Write-Output 完成
pause