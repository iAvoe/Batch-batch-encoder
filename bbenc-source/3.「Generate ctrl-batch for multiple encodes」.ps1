...
Read-Host "Multiple encoding mode only specifies the path to import files to encode, which require manually adding filenames into generated controller batch`r`nx264 usually comes with libav, unlike x265, therefore x265 usually exports .hevc raw-streams instead of .mp4. Press Enter to proceed"
...
Do {Switch (Read-Host "`r`nChoose how to specify filename of encoding exports [A: Input manually | B: Copy from an existing file | C: $vidEXP]`r`nNote that PowerShell misinterprets square brackets next to eachother, e.g., [some][text]. Space inbetween them is required") {
        ...
        c { $vidEXP+='_$serial'}  #Single quotes are used to prevent variable being expanded (becomes static value), comment this out in singular encoding batch mode
        default {Write-Warning "Bad input, try again"}
    }
} While (($vidEXP.Contains("$serial")) -eq $false)
...
::@set x265VarAAA=--crf 22 "+$x265VarNosChar[0]+"
::@set x265VarAAB=--crf 23 "+$x265VarNosChar[0]+"
::@set x265VarAAC=--crf 24 "+$x265VarNosChar[0]+"
