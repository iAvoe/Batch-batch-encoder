...
Read-Host "Multiple encoding mode only specifies the path to import files to encode, therefore manually adding files into generated controller batch is needed`r`nx264 usually comes with libav, unlike x265, therefore x265 usually exports .hevc raw-streams instead of .mp4. Press Enter to proceed"
...
#Do {[int]$qty = (Read-Host -Prompt "Specify the previous amount of generated [individual encoding batches]. Range from 1~15625")
...
Read-Host "`r`nPress Enter to open a selecting window that locates [path for exporting batch files], it may pop up at rear of current window."
...
Read-Host "Press Enter to proceed open a selection window to [locate path to export encoded files]"
...
Do {$IMPchk=$vidIMP=$vpyIMP=$avsIMP=$apmIMP=""
    Switch (Read-Host "The previously selected pipe upstream program was [A: ffmpeg | B: vspipe | C: avs2yuv | D: avs2pipemod]") {
        a {$IMPchk="a"; Write-Output "`r`nSelected ffmpeg-----video source. Opening a window to [locate a file to encode]`r`n"; $vidIMP=whichlocation} #Single-encode only, procedure is different from multi-encode mode
        b {$IMPchk="b"; Write-Output "`r`nSelected vspipe------.vpy source. Opening a window to [locate a file to encode]`r`n"; $vpyIMP=whichlocation}
        c {$IMPchk="c"; Write-Output "`r`nSelected avs2yuv-----.avs source. Opening a window to [locate a file to encode]`r`n"; $avsIMP=whichlocation}
        d {$IMPchk="d"; Write-Output "`r`nSelected avs2pipemod-.avs source. Opening a window to [locate a file to encode]`r`n"; $apmIMP=whichlocation}
        default {Write-Warning "Bad input, try again"}
    }
} While ($IMPchk -eq "")
...
Write-Output "`r`nPath/File selected under multi/single-encode mode is $impEXTc`r`n"
...
} #Note: Import paths' variable: $impEXTa; Import files' variable: $impEXTc; Export path variable: $fileEXP

#「Bootstrap G」Avs2pipemod requires avisynth.dll, which needs to be added to commandline
if ($IMPchk -eq "d") {
    Read-Host "Press Enter to proceed open a selection window for Avs2pipemod to [Select avisynth.dll],  it may pop up at rear of current window"
    $apmDLL=whereisit
...
if ($IMPchk -eq "a") {$impEXTc=$vidIMP}
    ...
    Read-Host "Press Enter to proceed open a window to [import a video sample for ffprobe to analyze]. Note that ffprobe cannot analyze .vpy/.avs files"
    $impEXTc=whereisit
}
...
Do {Switch (Read-Host "`r`nChoose how to specify filename of encoding exports [A: Input manually | B: Copy from an existing file | C: $vidEXP]`r`nNote that PowerShell misinterprets square brackets next to eachother, e.g., [some][text]. Space inbetween them is required") {
        a { Do {$vidEXP=Read-Host "`r`nSpecify filename to export (w/out file extension). Place episode conter `$serial in desired location.`r`n`r`n`$serial should be padded from trailing alphabets. e.g., [Zzz] Memories – `$serial (BDRip 1764x972 HEVC)"
                if (($vidEXP.Contains("`$serial")) -eq $false) {Write-Warning "Missing variable `$serial, No value entered, Or detecting illegal characters / | \ < > : ? * `""}
        ...
        b { Write-Output "Opening a selection window to [get filename from a file]"
            $vidEXP=whereisit
            $vidEXP+="_$serial" #Comment this out in singular encoding batch mode
            Write-Output "`r`nIn multi-encode mode, choosing A will add a trailing counter in filename`r`n"
        }
        c { $vidEXP+="_$serial"}  #Comment this out in singular encoding batch mode
        default {Write-Warning "Bad input, try again"}
    }
} While (($vidEXP.Contains("$serial")) -eq $false)
Write-Output "√ Added exporting filename $vidEXP`r`n"

#「BootstrapK, L」1: Specify file extention based on x264/5. 2: For x265, ddd pme/pools based on cpu core count & motherboard node count.
#Extra filtering x265 that usually doesn't come with libav (cannot export MP4), x264 usually comes with libav but does not support pme/pools
...
elseif ($ENCops -eq "a") {
    ...
    if ($cores -gt 11) {$pme="--pme"; Write-Output "Detecting processor core count is greater than 12`r`n√ Added x265 option: --pme"}
    ...
    if ($procNodes -gt 1) {Write-Output "Detected $procNodes installed processors`r`n√ Added x265 option: $pools"}
} 

Set-PSDebug -Strict
$utf8NoBOM=New-Object System.Text.UTF8Encoding $false #export batch file w/ utf-8NoBOM text codec

#Note: Import paths' variable: $impEXTa; Import file variable: $impEXTc; Export path variable: $fileEXP
...
#「Initialize」ffmpeg, vspipe, avs2yuv, avs2pipemod Variable optoins, this works differently compared to multi-encode mode
...
$ctrl_gen="::「Compatible with UTF-8」opt-out from ANSI code page
::「Safe for with multiple runs」Achieved by set+endlocal, works both at stopping during encoding and controlling batch
::「Compatible localized CLI language」Implementation failed, the batch file must run with code page 65001
::「Startup」Disable prompt input display, 5s sleep
chcp 65001 && @echo off && timeout 5
setlocal

::「Non-std exiting」Clean up used variables with taskkill /F /IM cmd.exe /T. Otherwise it may cause variable contamination
...
::「ffmpeg debug」Delete -loglevel 16
::「-thread_queue_size too small」Specity ffmpeg option -thread_queue_size<computing kbps+1000>, but the better practice is to replace ffmpeg
::「ffmpeg, vspipe, avsyuv, avs2pipemod fixed Parameters」
...
::「ffmpeg, vspipe, avsyuv, avs2pipemod Variable optoins」
...
::「x264/5 fixed Parameters」

@set x265ParA="+$x265ParA+"
@set x264ParA="+$x264ParA+"

::「x264/5 Variable optoins」Comment out during debugging

@set x265VarA="+$x265VarA+"
@set x264VarA="+$x264VarA+"

::「Debugging」x265 Variable options, comment out during normal use, variables have no trailing spaces
...
::「Encoding」Use commenting or deleting encode batches to skip undesired encode tasks

call enc_0S.bat

::「Finish」Perserve CMD prompt after finish, use /k instead of -k could skip printing of Windows build information
endlocal
cmd -k"
