...
function Set-UACRegistryValues {
    ...
    try {
        ...
        if ($backup) {
            ...
            Write-Host "已备份UAC设置到: $backupPath" -ForegroundColor Green
        }
        ...
        Write-Host "UAC设置已更新，需要重启系统才能生效。" -ForegroundColor Yellow
        return $true
    }
    catch {
        Write-Error "设置UAC失败: $_"
        return $false
    }
}
    ...
    try {
        ...
        foreach ($processor in $processors) {
            $info = @(
                ...
                "L2缓存: $([math]::Round($processor.L2CacheSize/1KB, 2)) MB",
                "L3缓存: $([math]::Round($processor.L3CacheSize/1KB, 2)) MB",
