...
.SYNOPSIS
    ffprobe 視頻源分析腳本
.DESCRIPTION
    分析源視頻並導出到 %USERPROFILE%\temp_v_info(_is_mov).csv: 總幀數，寬，高，色彩空間，傳輸特定等
.AUTHOR
...
# 加載共用代碼，包括 $utf8NoBOM、Get-QuotedPath、Select-File、Select-Folder...
. "$PSScriptRoot\Common\Core.ps1"

# 同時生成佔位 AVS/VS 腳本到 %USERPROFILE%，從而在用戶暫無可用腳本的情況下頂替
function Get-BlankAVSVSScript {
    ...
    $blankAVSScript = "LWLibavVideoSource($quotedImport) # 自動生成的佔位腳本，按需修改"
    # 生成 VapourSynth 內容（使用原始字符串 literal r"..." 以避免轉義問題）
...
function Main {
    Show-Border
    Write-Host ("ffprobe 源讀取工具，導出 " + $Global:TempFolder + "temp_v_info(_is_mov).csv 供後續步驟調用") -ForegroundColor Cyan
    Show-Border
    Write-Host ""
    ...
    $selectedType = $null
    do {
        Show-Info "選擇先前腳本所用的管道上游程序（確認源符合程序要求）："
    ...
    # 獲取上游程序代號（寫入 CSV）；為 Avs2PipeMod 導入必須的 DLL
    $upstreamCode = $null
    $Avs2PipeModDLL = $null
        ...
        'SVFI'         {
            ...
            do {
                if ($foundPath) { # 嘗試自動定位到的 SVFI 路徑（Select-File 能自動回退到 Desktop）
                    ...
                    $OneLineShotArgsINI = Select-File -Title "選擇 SVFI 渲染配置文件（.ini）" -IniOnly -InitialDirectory $foundPath
                }
                else { # DIY
                    $OneLineShotArgsINI = Select-File -Title "選擇 SVFI 渲染配置文件（.ini）" -IniOnly
                }
    ...
    # 定義變量
    $videoSource = $null # ffprobe 將分析這個視頻文件
    $scriptSource = $null # 腳本文件路徑，如果有則在導出的 CSV 中覆蓋視頻源
    ...
    if ($isScriptUpstream) {
        do {
            # 首先選擇視頻源文件（用於ffprobe分析）
            Show-Info "選擇（腳本引用的）視頻源文件（ffprobe 將分析此文件）"
            while ($null -eq $videoSource) {
                $videoSource = Select-File -Title "選擇視頻源文件（例如 .mp4/.mkv/.mov）"
                if ($null -eq $videoSource) { Show-Error "未選擇視頻文件" }
            }
        
            # 詢問用戶是否要生成無濾鏡腳本
            $mode = Read-Host "輸入 'y' 導入自定義腳本，輸入 'n' 或 Enter 為視頻源生成無濾鏡腳本"
        
            if ($mode -eq 'y') { # 導入自定義腳本
                do {
                ...
                # 注意：視頻源 $videoSource 仍然用於 ffprobe
            }
            # 生成無濾鏡腳本
            elseif ([string]::IsNullOrWhiteSpace($mode) -or $mode -eq 'n') {
                Show-Warning "`r`n AviSynth(+) 默認不自帶 LSMASHSource.dll（視頻導入濾鏡）請保證該文件存在，"
                Write-Host " AVS 安裝路徑為：C:\Program Files (x86)\AviSynth+\plugins64+\" -ForegroundColor Yellow
                Write-Host " 下載並解壓 64bit 版：https://github.com/HomeOfAviSynthPlusEvolution/L-SMASH-Works/releases" -ForegroundColor Yellow
    ...
    # SVFI：從 INI 文件中讀取視頻路徑，以及 task_id
    elseif ($OneLineShotArgsINI -and (Test-Path -LiteralPath $OneLineShotArgsINI)) { 
        # SVFI ini 文件中的視頻路徑（實際上內容為單行）：gui_inputs="{
        #     \"inputs\": [{
        #         \"task_id\": \"必須獲取並賦值到 CSV\",
        #         \"input_path\": \"X:\\\\Video\\\\\\u5176\\u5b83-\\u52a8\\u6f2b\\u753b\\u516c\\u79cd\\\\視頻.mp4\",
        ...
        Show-Info " 將嘗試從 SVFI 渲染配置 INI 中讀取視頻源路徑..."

        try { # 讀取 INI 並查找 gui_inputs 行
            ...
            try {
                ...
                Show-Warning "將導入其中的首個視頻源，忽略其它視頻源"
                ...
                Show-Success "從 SVFI INI 中解析到視頻源：$videoSource"

                # 驗證視頻文件是否存在
                if (-not (Test-Path -LiteralPath $videoSource)) {
                    Show-Error "視頻文件已不存在：$videoSource，請重新用 SVFI 生成 INI 文件"
                    Read-Host "按 Enter 退出"
                    return
                }
            }
        ...
        do {
            Show-Info "選擇要分析的視頻源文件"
            $videoSource = Select-File -Title "定位視頻文件，如視頻（.mp4/.mov/...）、RAW（.yuv/.y4m/...）"
            if (-not $videoSource) { 
                Show-Error "未選擇文件" 
                continue
            }
            
            Show-Success "已選擇視頻源文件：$videoSource"
            break
        }
        while ($true)
    ...
    if ($isMOV) {
        Show-Debug "`r`n導入視頻 $videoSource 的封裝格式為 MOV`r`n"
    }
    else {
        Show-Debug "`r`n導入視頻 $videoSource 的封裝格式非 MOV`r`n"
    }

    # 根據封裝文件類型選用 ffprobe 命令、定義文件名
    $ffprobeArgs =
        if ($isMOV) {@(
    ...
    # 由於 ffprobe 讀取不同源所產生的列數不一，導致讀取額外插入的信息隨機錯位，因此需要獨立的 CSV（s_info）來儲存源信息
