...
function Get-UACRegistryValues {
    try {
        ...
        if (-not (Test-Path -Path $regPath)) {
            throw "UAC 註冊表損壞（如果你看到這行，則建議儘快運行 Windows 修復）"
        }
...
function Set-UACRegistryValues {
    ...
    try {
        ...
        Write-Host "UAC 設置已更新，需要重啓系統才能生效。" -ForegroundColor Yellow
        return $true
    }
    catch {
...
function Show-HardwareInformation {
    ...
    try {
        ...
        $info = @(
            ...
            "發佈日期: $(($bios.ReleaseDate).ToString('yyyy-MM-dd'))"
        )
        
        $info | ForEach-Object { Write-Host "  $_" }
    }
    catch {
        Show-Warning "無法獲取BIOS信息: $_"
    }
    ...
    try {
        ...
        foreach ($processor in $processors) {
            $info = @(
                ...
                "線程數: $($processor.NumberOfLogicalProcessors)",
                "L2 緩存: $([math]::Round($processor.L2CacheSize/1KB, 2)) MB",
                "L3 緩存: $([math]::Round($processor.L3CacheSize/1KB, 2)) MB",
    ...
    try {
        ...
        Show-Success "總內存: $totalMemory GB"
    }
    catch {
        Show-Warning "無法獲取內存信息: $_"
    }
}
#endregion
...
"@ -ForegroundColor Yellow
        ...
        switch ($choice.ToUpper()) {
            'A' {
                ...
                if (-not (Test-Administrator)) {
                    ...
                    Write-Host " 你也可以在控制面板/設置裏自行調整" -ForegroundColor Yellow
            ...
            'C' {
                ...
                if (Set-UACRegistryValues -Values $uacValues) {
                    Show-Success "UAC 已恢復到默認設置，需要重啓系統生效"
                }
    ...
    # 5. 顯示系統硬件信息
    Show-Info "系統硬件信息" -ForegroundColor
    ...
    if ($currentUAC -and $currentUAC.EnableLUA -eq 0) {
        Show-Warning "注意：UAC 當前已禁用，建議在處理完畢後重新啓用以提高安全性"
    }
