# PowerShell 批处理智能视频编码器

为自动化「编码参数配置」而开发的脚本软件，解决了视频压缩软件需要手动配置编码参数、以及纯命令行操作需要手动输入路径的痛点。本工具会根据电脑硬件规格、视频属性、程序格式、脚本补充、用户需求等规则为前提配置出编码命令（批处理）。此工具几乎消灭了视频压制的学习成本与配置操作的时间成本，快点上手试试吧~
...
- [x] 按步骤顺序拆分的脚本：复用一切已完成的工作，最大限度避免从头开始重做
- [x] 深度定制编码参数：根据电脑硬件规格，用户需求，视频属性实现尽可能符合需求的压制配置
...
- 步骤 2 改良了导入系统
  ...
  - 添加了环境变量的 ffmpeg、avs2yuv、avs2pipemod、x264、x265、SvtAv1EncApp 的模糊匹配功能
